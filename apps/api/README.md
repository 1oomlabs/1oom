...
자연어 한 줄로 자동화 워크플로우를 만들고, KeeperHub로 실제 cron/조건부 실행을 맡기고, 마켓플레이스에서 공유까지 한 번에 처리하는 API 서버. Anthropic Claude Haiku로 의도를 추출하고, Zod 스키마로 런타임 검증한 뒤, KeeperHub HTTP API로 등록한다.
...
사용자가 자연어로 입력하면 프론트(A)가 `POST /api/workflows`로 백엔드(B)에 전달한다. 백엔드 안에서 LLM이 의도를 추출하고, 템플릿과 합쳐 워크플로우 객체로 빌드·검증, 인메모리 저장, KeeperHub에 등록까지 한 번에 처리해서 `keeperJobId`를 받아온다.
...
│   ├── env.ts                     # 환경변수 zod 검증
│   ├── store.ts                   # 워크플로우 인메모리 store
│   ├── marketplaceStore.ts        # 마켓 listing 인메모리 store
...
총 18개 endpoint. 모든 응답은 JSON. 에러는 표준 shape (섹션 3-6 참고).
...
- 섹션 2: 워크플로우 풀 라이프사이클 (8단계)
- 섹션 3: 마켓플레이스 (5개)
- 섹션 4: 에이전트 (3개)
- 섹션 5: 친절 에러 검증 (의도적 잘못된 입력)
...
### ④ Mergeable quality — 코드 품질 + 문서

- 패키지별 typecheck 통과 (api / keeperhub-client)
- `demo.http` 시나리오로 심사위원이 동일 시연 가능
...
- **인메모리 store**: 데모 직전 `pnpm dev` 재시작 금지 (휘발됨)
